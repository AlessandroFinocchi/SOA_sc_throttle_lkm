...
- **Sospensione del thread**: Qualora un thread non riesce ad ottenere un token, secondo l'algoritmo token-bucket, viene posto in stato di sleep sulla wait-event queue.

- **Risveglio del thread**: Allo scadere di cadauna finestra temporale, dalla durata di 1 secondo, il bucket viene riempito di tutti i suoi token, e contestualmente tutti i thread vengono risvegliati in modo da poter competere per i suddetti token in modo da poter ritornare nello stato running.
...
- Se la macro `WEQ_UNINT` **non è definita**, le wait-event queue saranno **interrompibli**.
- Se la macro `WEQ_UNINT` **è definita**, le wait event queue saranno **non interrompibili**.

A coronare la gestione dei segnali c'è anche il meccanismo di **Hijacking del flusso d'esecuzione** aggiunto nel [Sottosistema Kprobes](probes.md) (vedere la relativa sottosezione), in modo da rendere robusto il throttling anche in caso di wait-event queue interrompibili.
